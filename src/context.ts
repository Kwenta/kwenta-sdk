...
export interface IContext {
	...
	logError?: (error: Error, skipReport?: boolean) => void;
}
...
export default class Context implements IContext {
	...
	get multicallProvider() {
		return new EthCallProvider(this.context.networkId, this.context.provider);
	}
	...
	public logError(error: Error, skipReport?: boolean) {
		if (!this.context.logError) {
			throw new Error('Error logging function not defined');
		}

		return this.context.logError(error, skipReport);
	}
